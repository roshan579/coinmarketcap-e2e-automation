package com.cmcp.test.framework.report;

import com.cmcp.test.framework.model.ReportModel;
...
public class ConsolidatedReport {
    ...
    private static final String REASON = "error_message";
    private static final String DURATION = "duration";
    private static final String STATUS = "status";
    private static final String PASSED = "passed";
    private static final String JOBS_PATH = "/home/.jenkins/jobs/";
    private static final String JOB_BUILD_DIR = "/builds";
    private static final String NEXT_BUILD_NUMBER_FILE_NAME = "/nextBuildNumber";

    private com.cmcp.test.framework.report.MergeReports mergeReports;
    ...
    public ConsolidatedReport() {
        mergeReports = new com.cmcp.test.framework.report.MergeReports();
    ...
    public void copyJsonFilesFromJobs() {
        ...
        File destDir = new File(com.cmcp.test.framework.report.MergeReports.FILE_PATH);
        if (!destDir.exists()) {
            destDir.mkdirs();
        }
    ...
    public void getStatisticsFromJsons() throws Exception {
        List<String> jsonFileList = mergeReports.getJsonFiles(com.cmcp.test.framework.report.MergeReports.FILE_PATH);
        ...
        try {
            ...
            for (int i = 0; i < scenarios.size(); i++) {
                ...
                if(!getElementKeyword(scenario).equalsIgnoreCase(BACKGROUND)) {
                    ...
                    reportModel.setStatus(getStatus(scenario));
                    reportModel.setExecutionTime(getDuration(scenario));
                    reportModel.setReason(getReason(scenario));
            ...
            reportModelException.setReason("due to : "+ex.getMessage());
            reportModelException.setExecutionTime(new Long(0));
    ...
    public String getTestName(JSONObject jsonObject) {
        ...
        for (int i = 0; i < jsonTags.size(); i++) {
            ...
            tags = tags + " " + (String) jsonTag.get(NAME);
        }
        return tags.trim();
    }
    ...
    public String getReason(JSONObject jsonObject) {
        ...
        for (int i = 0; i < jsonSteps.size(); i++) {
            ...
            String jsonStatus = (String) jsonResult.get(STATUS);
            if (!jsonStatus.equalsIgnoreCase(PASSED)) {
                reason = (String) jsonResult.get(REASON);
                break;
            }
        }
        return reason;
    ...
    public Long getDuration(JSONObject jsonObject) {
        Long totalDuration = new Long(0);
        JSONArray jsonSteps = (JSONArray) jsonObject.get(STEPS);
        for (int i = 0; i < jsonSteps.size(); i++) {
            if(jsonSteps.get(i) != null){
                JSONObject jsonStep = (JSONObject) jsonSteps.get(i);
                if(jsonStep.get(RESULT) != null){
                    JSONObject jsonResult = (JSONObject) jsonStep.get(RESULT);
                    if(jsonResult.get(DURATION) != null){
                        Long duration = (Long) jsonResult.get(DURATION);
                        totalDuration = totalDuration + (duration) / (1000 * 1000);
                    }
                }
            }
        }
    ...
    private String getTimeInHoursMinutesSecondsFormat(Long diff){
        long diffMilliSeconds = diff / (1) % 1000;
        long diffSeconds = diff / (1 * 1000) % 60;
        long diffMinutes = diff / (60 * 1000) % 60;
        long diffHours = diff / (60 * 60 * 1000) % 24;
        String returnString = "";
        if(diffHours > 0){
            returnString = returnString + diffHours+"h ";
        }
        if(diffMinutes > 0){
            returnString = returnString + diffMinutes+"m ";
        }
        if(diffSeconds > 0){
            returnString = returnString + diffSeconds+"s ";
        }
        if(diffMilliSeconds > 0){
            returnString = returnString + diffMilliSeconds+"ms";
        }
        return returnString;
    }
    ...
    public void createReport() throws IOException {
        ...
        for (ReportModel reportModel : jsonReports) {
            ...
            totalExecutionTime = totalExecutionTime + reportModel.getExecutionTime();
            ...
            table2Rows.append("</td><td>");
            table2Rows.append(reportModel.getReason());
            table2Rows.append("</td><td>");
            table2Rows.append(getTimeInHoursMinutesSecondsFormat(reportModel.getExecutionTime()));
        ...
        scriptTag.append("\t\t<script type=\"text/javascript\" src=\"test-classes/canvasjs.min.js\"></script>");
        percentage = (double) passedScenarioCount / totalScenarioCount * 100;
        StringBuilder html = new StringBuilder();
        html.append("<!DOCTYPE html>");
        html.append("<html lang=\"en\">");
        html.append("<head>");
        html.append("<meta charset=\"UTF-8\">");
        html.append("<title>Consolidated ASUI automation test report</title>");
        html.append(scriptTag);
        html.append("</head>");
        html.append("<body bgcolor=\"#ffffff\">");
        html.append("<h1>Consolidated test report</h1>");
        html.append("<h2>Date : ");
        ...
        table1.append("<tr><th>Total</th><th>Passed</th><th>Failed</th><th>Passed Percentage</th><th>Execution Time</th></tr>");
        ...
        table1.append("</td><td>");
        table1.append(getTimeInHoursMinutesSecondsFormat(totalExecutionTime));
        ...
        table2.append("<th>Status</th>");
        table2.append("<th>Reason</th>");
        table2.append("<th>Time</th></tr>");
    ...
    public static void main(String a[]) {
        com.cmcp.test.framework.report.ConsolidatedReport consolidatedReport = new com.cmcp.test.framework.report.ConsolidatedReport();
        try {
            consolidatedReport.copyJsonFilesFromJobs();
            consolidatedReport.getStatisticsFromJsons();
        } catch (Exception ex) {
            System.out.println("Report could not be created" + ex.getMessage());
        }
    }
